...
Style = (function() {
  ...
  Style.prototype.generate = function(options) {
    ...
    styles = [this.css(this.selector, ["  background-image: url( '" + relativeImagePath + "' )", "  background-repeat:  no-repeat", "  background-size: " + (width / pixelRatio) + "px " + (height / pixelRatio) + "px"])];
    if (pixelRatio === 1) {
